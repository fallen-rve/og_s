...
function og_s_font_url() {
	...
	*/
	$roboto = _x( 'on', 'Roboto font: on or off', 'og_s' );
	$open_sans = _x( 'on', 'Open Sans font: on or off', 'og_s' );
...
function og_s_scripts() {
	...
	wp_enqueue_style( 'og_s-google-font', og_s_font_url(), array(), null );
...
add_action( 'wp_enqueue_scripts', 'og_s_scripts' );
...
if ( class_exists( 'WDS_Simple_Page_Builder' ) && version_compare( WDS_Simple_Page_Builder::VERSION, '1.6', '>=' ) ) :
	 ...
	 */
	function og_s_enqueue_page_builder_scripts() {
	...
	add_action( 'wds_page_builder_after_load_parts', 'og_s_enqueue_page_builder_scripts' );
